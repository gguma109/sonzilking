...
async function ensureProfileColumns(db) {
  ...
  for (const column of ['businessName', 'representativeName', 'registrationNumber', 'businessEmail', 'phone', 'bankAccount']) {
...
function publicUser(user) {
  return { id: user.id, username: user.username, name: user.name, nickname: user.nickname || '', recoveryEmail: user.recoveryEmail || '', createdAt: user.createdAt,
    businessName: user.businessName || '', representativeName: user.representativeName || '', registrationNumber: user.registrationNumber || '', businessEmail: user.businessEmail || '', phone: user.phone || '', bankAccount: user.bankAccount || '' };
}
...
export async function onRequestGet({ env, data }) {
  try {
    ...
    const user = await db.prepare('SELECT id, username, name, nickname, recoveryEmail, createdAt, businessName, representativeName, registrationNumber, businessEmail, phone, bankAccount FROM users WHERE id = ?').bind(data.userId).first();
...
export async function onRequestPut({ env, request, data }) {
  try {
    ...
    if (newPassword) {
      if (!currentPassword) return Response.json({ success: false, error: '현재 비밀번호를 입력해주세요.' }, { status: 400, headers: CORS });
    ...
    await db.prepare('UPDATE users SET name = ?, nickname = ?, recoveryEmail = ?, businessName = ?, representativeName = ?, registrationNumber = ?, businessEmail = ?, phone = ?, bankAccount = ?, passwordHash = ? WHERE id = ?')
      .bind(name, nickname, recoveryEmail, businessName, representativeName, registrationNumber, businessEmail, phone, bankAccount, passwordHash, data.userId).run();
    const updated = await db.prepare('SELECT id, username, name, nickname, recoveryEmail, createdAt, businessName, representativeName, registrationNumber, businessEmail, phone, bankAccount FROM users WHERE id = ?').bind(data.userId).first();
